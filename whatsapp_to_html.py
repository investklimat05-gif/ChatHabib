...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .search-panel .text-search input {
            flex: 1;
            border: none;
            padding: 6px;
            font-size: 14px;
        ...
        .search-panel .text-search button {
            background: none;
            border: none;
            cursor: pointer;
            font-size: 16px;
        ...
        .search-panel .text-search .nav-buttons {
            display: flex;
            gap: 4px;
        }
        .search-panel .text-search .clear-search {
            font-size: 18px;
        }
        .chat-wrapper {
...
<div class="search-panel">
    <input type="text" id="date-search" class="date-input" placeholder="Выберите дату">
    <div class="text-search">
        <input type="text" id="text-search-input" placeholder="Поиск в чате">
        <div class="nav-buttons">
            <button id="search-prev" disabled>▲</button>
            <button id="search-next" disabled>▼</button>
        </div>
...
<script>
    ...
    messages.forEach(msg => {
        ...
        // Сохраняем оригинальный текст для поиска
        const originalText = msg.text || "";
        msgDiv.setAttribute('data-original-text', originalText);
        msgDiv.innerHTML = `
            <div class="bubble ${isMe ? 'bubble-right' : 'bubble-left'}">
                ...
                <div class="text">${msg.text ? msg.text.replace(/[&<>]/g, function(m){if(m === '&') return '&amp;'; if(m === '<') return '&lt;'; if(m === '>') return '&gt;'; return m;}) : ''}</div>
                ${mediaHtml}
    ...
    function clearHighlights() {
        const textDivs = document.querySelectorAll('.message .text');
        textDivs.forEach(div => {
            const original = div.parentElement.parentElement.getAttribute('data-original-text');
            if (original !== null) {
                div.innerHTML = original.replace(/[&<>]/g, function(m){if(m === '&') return '&amp;'; if(m === '<') return '&lt;'; if(m === '>') return '&gt;'; return m;});
            }
        });
        matches = [];
        currentMatchIndex = -1;
    ...
    function performSearch() {
        ...
        textDivs.forEach((div, idx) => {
            const originalText = div.parentElement.parentElement.getAttribute('data-original-text');
            if (!originalText) return;
            const escapedOriginal = originalText.replace(/[&<>]/g, function(m){if(m === '&') return '&amp;'; if(m === '<') return '&lt;'; if(m === '>') return '&gt;'; return m;});
            const newHtml = escapedOriginal.replace(regex, '<mark>$1</mark>');
            if (newHtml !== escapedOriginal) {
                div.innerHTML = newHtml;
                matches.push({ div, originalText });
                matchCount++;
            } else {
                div.innerHTML = escapedOriginal;
            }
        });
        if (matches.length > 0) {
            currentMatchIndex = 0;
    ...
    function scrollToMatch(index) {
        ...
        const matchDiv = matches[index].div;
        const wrapper = document.getElementById('scroll-wrapper');
        const targetRect = matchDiv.getBoundingClientRect();
        const wrapperRect = wrapper.getBoundingClientRect();
        let targetTop = targetRect.top - wrapperRect.top + wrapper.scrollTop - 80; // отступ сверху
        wrapper.scrollTo({ top: targetTop, behavior: 'smooth' });
        // Подсветка текущего совпадения (дополнительно)
        matchDiv.querySelector('mark')?.scrollIntoView({ block: 'center', behavior: 'smooth' });
    }
